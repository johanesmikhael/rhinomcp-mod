...
The stair is the one worth running twice. It is the same three blocks either way:
...
point is free to rotate about it, so the same stack is a mechanism. Neither is wrong: they
are answers to different questions about how the blocks are connected.
...
so the demo files and the tested cases cannot drift apart. It writes to whatever document
