...
public partial class RhinoMCPModFunctions
{
    public JObject SelectObjects(JObject parameters)
    {
        ...
        var names = hasName ? castToStringList(filters.SelectToken("name")) : null;
        ...
        if (filtersType == "and")
            foreach (var obj in objects)
            {
                ...
                if (hasName && (names == null || names.Count == 0 || !names.Contains(obj.Name))) continue;
            ...
            foreach (var obj in objects)
            {
                ...
                if (hasName && names != null && names.Contains(obj.Name)) attributeMatch = true;
