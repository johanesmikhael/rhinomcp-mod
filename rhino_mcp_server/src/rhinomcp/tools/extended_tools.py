...
async def graph_display(
    ...
    The overlay draws what the graph found: which elements touch, where they
    touch, and the bearing surface measured at each contact. It is the check
    that the topology read off the model is the topology that was meant - a
    contact the graph never found is not there for anything downstream, and
    a bearing measured on the wrong plane faces the wrong way.
