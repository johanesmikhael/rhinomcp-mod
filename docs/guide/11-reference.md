...
other route has no equivalent. Pages 02-05 are not yet written; their tools are listed here.

## MCP tools

| tool | does | rhino command | page |
| --- | --- | --- | --- |
| `get_document_info` | document metadata and an object listing at `detail="inventory"`, `"summary"` or `"full"`; `limit`/`offset` paging; `bbox` + `bbox_mode` scope | - | 02 |
| `get_object_info` | one object by `id` or `name`; `geometry_detail` as below | `What` | 02 |
| `get_objects_info` | several objects by selectors; `geometry_detail="bbox"` (world box), `"obb_pose"` (oriented box + pose), `"ortho3"` (three outlines); `include_attributes`, `include_world` | - | 02 |
| `get_selected_objects` | id, name, type, layer of the current selection | - | 02 |
| `select_objects` | select by `ids`, `names`, `layer` or `type` (any match) | `SelLayer`, `SelName` | 03 |
| `deselect_all` | clear the selection | `SelNone` | 03 |
| `create_objects` | create points, lines, polylines, curves, circles, arcs, ellipses, boxes, spheres, cones, cylinders, surfaces; optional `translation`, `rotation`, `scale`, `color`, `name` | the drawing commands | 03 |
| `copy_objects` | copy by id or name with a translation | `Copy` | 03 |
| `modify_objects` | rename, recolour, move to a layer, translate, rotate, scale, show or hide; `all=True` for every object | `Properties`, `Move`, `Scale` | 03 |
| `delete_objects` | delete by `ids` or `names`; `confirm=True` required | `Delete` | 03 |
| `create_layer` | new layer with `name`, optional `color` and `parent` | `Layer` | 03 |
| `delete_layer` | delete by `guid` or `name` | `Layer` | 03 |
| `rename_layer` | rename by `id` | `Layer` | 03 |
| `get_or_set_current_layer` | read the current layer, or set it by `guid` or `name` | `Layer` | 03 |
| `move_objects_to_layer` | move `ids` to `layer` | `ChangeLayer` | 03 |
| `get_layer_states` | visibility, lock and colour of every layer | `Layer` | 03 |
| `save_layer_state` | remember the current layer visibility and locks under `name`; in memory, gone when the plugin unloads | `LayerStateManager` | 03 |
| `restore_layer_state` | apply a saved state | `LayerStateManager` | 03 |
| `get_materials` | every material in the document | `Materials` | 03 |
| `create_material` | a material with a diffuse colour | `Materials` | 03 |
| `set_object_material` | assign by `material_name` or `material_index` | `Properties` | 03 |
| `get_object_materials` | the material on each of `ids` | `Properties` | 03 |
| `rotate_objects` | rotate about a pivot by a 3x3 matrix, per object or `all=True` | `Rotate3D` | 04 |
| `invert_rotation_matrix` | the inverse of a 3x3 rotation, to undo one | - | 04 |
| `rebase_objects_pose` | take the current placement as the canonical pose without moving anything | - | 04 |
| `reset_objects_pose` | return objects to their canonical pose | - | 04 |
| `capture_view` | frame `ids`, the selection or everything, capture a PNG in a `display_mode` at a `resolution`, optional explicit camera and lens; the view is restored after | `ViewCaptureToFile` | 05 |
| `get_viewport_info` | camera, projection, lens and display mode of every viewport | `ViewportProperties` | 05 |
| `zoom_to_objects` | zoom the active viewport to `ids` | `Zoom Selected` | 05 |
| `get_named_views` | the named views with their cameras | `NamedView` | 05 |
| `save_named_view` | save the active camera as `name` | `NamedView` | 05 |
| `restore_named_view` | restore `name` into a viewport | `NamedView` | 05 |
| `delete_named_view` | delete `name` | `NamedView` | 05 |
...
| `mcpmodobb` | draw oriented bounding boxes and projection profiles | `On` / `Off` / `Toggle` / `Status` | `mcpmodobb Off` | - | 04 |
